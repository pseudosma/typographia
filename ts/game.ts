...
import { updateTextPosition } from "./gameLogic";
import { FontLoader, NewFontLoader } from "./fontLoader";
import { Configurable, LoadGameConfig } from "./gameConfig";
import { TextLoader, NewTextLoader } from "./textLoader";

interface BabylonGame {
...
interface ScrollingGame {
    lastX: number;
}

interface TextBasedGame extends ScrollingGame{
    fontLoader: FontLoader;
    textLoader: TextLoader;
    UpdateTextPosition: (scene, camera, lastX, text, fontLoader) => number;
}

export class Game implements BabylonGame, TextBasedGame, Configurable {
    canvas = document.getElementById("renderCanvas") as HTMLCanvasElement;
    engine = new Engine(this.canvas);
    scene = new Scene(this.engine);
    camera = new UniversalCamera("camera1", new Vector3(0, 0, -10), this.scene);
    light = new HemisphericLight("light1", new Vector3(0, 1, 0), this.scene);
    ...
    constructor() {
        ...
        //add shader to the scene
        new ShaderBuilder(this.scene);
    }

    public CreateScene(): void {
         //TODO: Add author, title, and chapter info
        
    //     const t = this.textLoader.LoadText(
    //         fetch,
    //         this.config, 
    //         this.textLoader.text, 
    //         this.textLoader.chapters
    //     );
    //     const fl = this.fontLoader;
    //     const s = this.scene;
    //     const e = this.engine;
    //     const tl = this.textLoader;
    //     const p = this.fontLoader.LoadFonts(
    //         SceneLoader,
    //         this.scene,
    //         this.config.graphicsPath,
    //         this.config.graphicsFiles.font
    //     );
    //     Promise.all([t, ...p]).then(() => {
    //         //Only after all meshes are loaded, then we can try to instance text.
    //         for (let i=0; i<41; i++) {
    //             fl.InstanceLetter(s, tl.text.content[i], i);
    //         }
    //         e.hideLoadingUI();
    //       });
        ...
        const p = this.fontLoader.LoadFonts(SceneLoader, this.scene, this.config.graphicsPath, this.config.graphicsFiles.font);
    ...
    public Run(): void {
        this.engine.runRenderLoop(() => {
            this.lastX = this.UpdateTextPosition(
                this.scene, 
                this.camera, 
                this.lastX, 
                this.textLoader.text, 
