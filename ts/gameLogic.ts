...
import { GameConfig, LoadGameConfig } from "./gameConfig";
...
export class Game implements BabylonGame, TextBasedGame, ScrollingGame, Configurable {
    ...
    public Run(): void {
        this.engine.runRenderLoop(() => {
            ...
            if (this.lastX < x && max > -1) {
                ...
                if (i == null) {
                    this.fontLoader.InstanceLetter(this.scene, this.text.content[max], max);
                }
            }
            if (this.lastX > x && min <= this.text.content.length) {
                ...
                if (min > -1) {
                    ...
                    if (i == null) {
                        this.fontLoader.InstanceLetter(this.scene, this.text.content[min], min);
                    }
                }
            }
            ...
            for (let i = max + 1;max + 3 > i; i++) {
                this.fontLoader.DeinstanceLetter(this.scene, i)
            }
